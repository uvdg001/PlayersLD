...
import type { Tournament, Match, Opponent, Venue, MyTeam, Page } from '../../types.ts';
...
interface FixturePageProps {
    ...
    onDeleteMatch: (matchId: number) => void; // Nueva prop
    isAdmin: boolean;
}
...
export const FixturePage: React.FC<FixturePageProps> = ({ tournaments, myTeam, matches, opponents, venues, setCurrentPage, setSelectedMatchId, onOpenMatchForm, onDeleteMatch, isAdmin }) => {
                            ...
                            {tournamentMatches.length > 0 ? (
                                <div className="space-y-3">
                                    {tournamentMatches.map(match => (
                                        <div 
                                            key={match.id} 
                                            className="flex items-center justify-between p-3 bg-gray-100 dark:bg-gray-700 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors group"
                                        >
                                            <div 
                                                onClick={() => {
                                                ...
                                                className="flex-grow grid grid-cols-1 md:grid-cols-5 gap-4 items-center cursor-pointer"
                                            >
                                            ...
                                            {isAdmin && (
                                                <div className="pl-4 flex items-center gap-2">
                                                    <button 
                                                        onClick={() => onOpenMatchForm({ tournamentId: tournament.id, match: match })}
