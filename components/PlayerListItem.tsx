...
import type { Player, PlayerMatchStatus } from '../types';
import { AttendanceStatus, PaymentStatus } from '../types';
...
const StatControl: React.FC<{ icon: string; value: number; onUpdate: (newValue: number) => void; max?: number }> = ({ icon, value, onUpdate, max }) => (
    <div className="flex items-center space-x-1 bg-gray-200 dark:bg-gray-600 p-1 rounded-md">
...
export const PlayerListItem: React.FC<PlayerListItemProps> = ({ player, playerStatus, onPlayerStatusChange, onPlayerPaymentChange, onPlayerStatsChange, playerShare, isAdmin }) => {
    ...
    const handleSavePayment = (newTotal: number) => {
        ...
        } else if (newTotal >= playerShare && playerShare > 0) {
        ...
        <div className={`p-3 bg-gray-100 dark:bg-gray-700 rounded-lg ${statusStyles[playerStatus.attendanceStatus]}`}>
            ...
            {isAdmin && playerStatus.attendanceStatus === AttendanceStatus.CONFIRMED && (
                <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-600 flex flex-wrap items-center gap-2">
                     <StatControl icon="⚽" value={playerStatus.goals || 0} onUpdate={(v) => handleStatChange('goals', v)} />
                     <StatControl icon="🟨" value={playerStatus.yellowCards || 0} onUpdate={(v) => handleStatChange('yellowCards', v)} max={2} />
                     <button onClick={handleRedCardToggle} className="flex items-center space-x-1 bg-gray-200 dark:bg-gray-600 p-1 rounded-md">
                        <span className="text-lg">🟥</span>
                        <div className={`w-5 h-5 rounded ${playerStatus.redCard ? 'bg-red-500' : 'bg-gray-400'}`}></div>
                     </button>
                     <StatControl icon="⏱️" value={playerStatus.quartersPlayed || 0} onUpdate={(v) => handleStatChange('quartersPlayed', v)} max={4} />
                </div>
            )}
        </div>
    );
